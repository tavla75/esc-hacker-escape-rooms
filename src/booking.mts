...
function getChallengeTitleById(id: string): string {
  try {
    ...
    if (saved) {
      const arr: oneChallenge[] = JSON.parse(saved);
...
function getChallengeById(id: string): oneChallenge | null {
  try {
    const saved = localStorage.getItem("savedChallenges");
    if (saved) {
      const arr: oneChallenge[] = JSON.parse(saved);
...
export function openBookingModal(challengeId: string): void {
...
function closeBookingModal(overlay: HTMLElement | null): void {
  if (!overlay) return;
...
function showThankYou(modalEl: HTMLElement, overlay: HTMLElement): void {
  modalEl.innerHTML = "";
...
export function openThankYouWindow(payload: any): void {
  ...
  const message = `
    ...
    payload.date
  } <br /><strong>Time:</strong> ${payload.time}</p>
        <p>We've sent a confirmation to: ${
  ...
  if (!newWin) {
    ...
    if (modalEl)
      showThankYou(
        modalEl,
        document.getElementById("booking-modal-overlay") as HTMLElement
      );
    return;
  }
